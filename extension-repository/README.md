...
The publish workflow writes the manifest to the reserved bucket key
`_meta/artifacts.json`. The Worker exposes it at `/artifacts.json`; it describes
the latest ABI-locked bundle and its R2 object paths, including SHA-256 values
for the runtime and extensions. The manifest is short-cacheable (60 seconds);
versioned extension objects remain immutable.
