...
static string EntryName(const QualifiedName &name) {
...
void DuckLakeSchemaEntry::Alter(CatalogTransaction catalog_transaction, AlterInfo &info) {
	...
	case AlterType::ALTER_TABLE: {
		...
		auto alter_name = EntryName(alter.GetQualifiedName());
		auto table_entry = GetEntry(catalog_transaction, CatalogType::TABLE_ENTRY, Identifier(alter_name));
		...
		if (alter.alter_table_type == AlterTableType::RENAME_TABLE) {
			...
			auto existing_table = GetEntry(catalog_transaction, CatalogType::TABLE_ENTRY, Identifier(new_table->name));
	...
	case AlterType::ALTER_VIEW: {
		...
		auto alter_name = EntryName(alter.GetQualifiedName());
		auto view_entry = GetEntry(catalog_transaction, CatalogType::VIEW_ENTRY, Identifier(alter_name));
		...
		if (alter.alter_view_type == AlterViewType::RENAME_VIEW) {
			// We must check if this view name does not yet exist.
			auto existing_view = GetEntry(catalog_transaction, CatalogType::VIEW_ENTRY, Identifier(new_view->name));
...
void DuckLakeSchemaEntry::DropEntry(ClientContext &context, DropInfo &info) {
	...
	auto info_name = info.GetQualifiedName().Name().GetIdentifierName();
	auto catalog_entry = GetEntry(GetCatalogTransaction(context), info.type, Identifier(info_name));
	if (!catalog_entry) {
		if (info.if_not_found == OnEntryNotFound::RETURN_NULL) {
			return;
		}
		throw InternalException("Failed to drop entry \"%s\" - could not find entry", info_name);
	}
	...
	if (local_set) {
		for (auto &entry : local_set->GetEntries()) {
			...
			if (entry_score > result.score) {
				...
				result.name = Identifier(entry.second->name);
				result.schema = this;
			}
		}
	}
