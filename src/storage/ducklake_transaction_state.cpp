...
void ConflictCheck(const case_insensitive_map_t<reference_set_t<CatalogEntry>> &created_changes,
	...
	for (auto &entry : created_changes) {
		...
		for (auto &catalog_ref : created_entry) {
			...
			string action =
				StringUtil::Format("create %s \"%s\" in schema \"%s\"", entry_type, catalog_entry.name.GetIdentifierName(), schema_name);
...
void DuckLakeTransactionState::CheckForConflicts(const TransactionChangeInformation &changes,
	...
	for (auto &entry : changes.created_tables) {
		...
		for (auto &table_ref : created_tables) {
			...
			string action =
				StringUtil::Format("create %s \"%s\" in schema \"%s\"", entry_type, table.name.GetIdentifierName(), schema_name);
...
string DuckLakeTransactionState::WriteSnapshotChanges(DuckLakeCommitState &commit_state,
	...
	for (auto &entry : changes.created_procedures) {
		...
		for (auto &created_procedure : entry.second) {
			...
			change_info.changes_made += schema_prefix + KeywordHelper::WriteQuoted(created_procedure.get().name.GetIdentifierName(), '"');
		}
	}
...
void GetNewProcedureInfo(DuckLakeCommitState &commit_state, reference<CatalogEntry> entry,
	...
	procedure_info.procedure_name = procedure_entry.name.GetIdentifierName();
