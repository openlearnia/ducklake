...
string DuckLakeTransactionState::WriteSnapshotChanges(DuckLakeCommitState &commit_state,
	...
	for (auto &entry : changes.created_procedures) {
		...
		for (auto &created_procedure : entry.second) {
			...
			change_info.changes_made +=
			    schema_prefix + DuckLakeUtil::SQLIdentifierToString(created_procedure.get().name.GetIdentifierName());
		}
	}
...
void GetNewProcedureInfo(DuckLakeCommitState &commit_state, reference<CatalogEntry> entry,
	...
	procedure_info.security_definer = procedure_entry.security_definer;
